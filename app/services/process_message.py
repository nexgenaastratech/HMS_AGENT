...
from app.services.agent import (
    run_agent,
    handle_pending_followup,
    handle_yes_no,
    handle_pending_room_selection,
)
...
def process_message(data: dict) -> None:
    try:
        ...
        if msg_type == "text":
            ...
            user_text = (interactive.get("list_reply") or interactive.get("button_reply", {})).get("title", "")
        
        if not user_text: return
        logging.info(f"[Webhook] {guest_name} ({wa_id}): {user_text}")
        
        if msg_type == "interactive":
            if handle_yes_no(wa_id, user_text, msg_type, msg.get("context", {}).get("id")):
                return
            if handle_pending_room_selection(wa_id, user_text):
                return
        response = run_agent(wa_id=wa_id, guest_name=guest_name, user_text=user_text)
